"""All the databses required for this app"""
...
class UserIntegration(models.Model):
    ...
    Fields:
        ...
        of your application is controlled by the user with the help of your application's default
        invoke name. Since a YA user is allowed to have multiple integrations with your application,
        YA will suffix the default invoke name for users who want to integrate more than once
        with your application, so that they can control the different integrations with their
...
class awss3(models.Model):

    """ This model is to store the AWS Credentials entered by user"""
    id = models.ForeignKey(UserIntegration, primary_key=True, on_delete=models.CASCADE)
    AWS_APIAccessKey = models.CharField(max_length=100)
    AWS_APISecretAccess = models.CharField(max_length=100)
    AWS_update_login_flag = models.BooleanField(max_length=2, default=False)
